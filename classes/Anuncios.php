...
class Anuncios{
    ...
    public function getAnuncios($filtros){
        global $pdo;
    ...
    public function getUltimosAnuncios($page,$perPage, $filtros){
        ...
        $sql = $pdo->prepare("SELECT *, 
                ...
                anuncios.id_categoria) as categoria FROM anuncios WHERE ".implode(' AND ',$filtrostring)." ORDER BY id DESC LIMIT $offset, $perPage");
     ...
     public function editAnuncio($titulo, $categoria, $valor, $descricao, $estado, $fotos, $id){
        ...
        if(count($fotos) > 0){
            for($i=0;$i<count($fotos['tmp_name']); $i++){
                ...
                $tmpname= md5(time().rand(0,9999)).'.jpg'; move_uploaded_file($fotos['tmp_name'][$i], 'assets/images/anuncios/'.$tmpname);
    ...
    public function getAnuncio($id){
        ...
        $sql = $pdo->prepare("SELECT *,
              (SELECT categorias.nome FROM categorias WHERE categorias.id =
                anuncios.id_categoria) as categoria,
              (SELECT usuarios.telefone FROM usuarios WHERE usuarios.id =
                anuncios.id_usuario) as telefone  
